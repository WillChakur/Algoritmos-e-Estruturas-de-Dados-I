...
int contar_nulos(Spa_Mat *mat);

int main() {
    int l, c, item, nulos, i;
    scanf("%d %d", &l, &c);
    ...
    for(i=0; i<l; i++){
        for(int j = 0; j<c; j++){
            scanf("%d", &item);
            if(item != 0){
                trocar_inserir_na_lista(item, j, mat->lin[i]);
            }
        }
    }

    nulos = contar_nulos(mat);
...
int contar_nulos(Spa_Mat *mat){
    ...
    for(i=0; i<mat->n_lin; i++){
        if(mat->lin[i]->head != NULL){
            ...
            quant_null += mat->n_col - aux;
        }
        else
            quant_null += mat->n_col;
        aux=0;
    }
